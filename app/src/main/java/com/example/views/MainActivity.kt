...
import com.example.views.ui.navigation.PsiloNavigation
...
/**
 * Main Activity for Psilo Android app.
 *
...
class MainActivity : ComponentActivity(), ComponentCallbacks2 {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        com.example.views.repository.Nip66RelayDiscoveryRepository.init(applicationContext)
        val settingsPrefs = getSharedPreferences("psilo_settings", MODE_PRIVATE)
        val prefetchEnabled = settingsPrefs.getBoolean("relay_discovery_prefetch", false)
        val hasAccount = getSharedPreferences("psilo_accounts", MODE_PRIVATE)
            .getString("all_accounts_json", null)?.isNotBlank() == true
        if (prefetchEnabled || hasAccount) {
            com.example.views.repository.Nip66RelayDiscoveryRepository.fetchRelayDiscovery()
        }
        ...
        lifecycle.addObserver(LifecycleEventObserver { _, event ->
            when (event) {
                Lifecycle.Event.ON_RESUME -> {
                    ...
                    com.example.views.repository.Nip66RelayDiscoveryRepository.refreshIfStale()
                }
        ...
        com.example.views.ui.settings.MediaPreferences.init(applicationContext)
