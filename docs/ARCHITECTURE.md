# Psilo Android — Architecture & State Flow Documentation

## Overview

Psilo is a Nostr client for Android built with Jetpack Compose, Kotlin Coroutines/Flows, and direct WebSocket connections to Nostr relays. The app follows an MVVM architecture with repository-backed data layers and a single-Activity Compose Navigation host.
...
- **`PsiloNavigation.kt`** → `NavHost` with all route composables
...
### AdaptiveHeader
- **Psilo logo menu:** Dropdown from title with All/Following, Latest/Popular, Topics navigation
...
| File | Purpose |
|------|---------|
| `PsiloNavigation.kt` | NavHost, all route composables, thread overlay logic |
...
| `AdaptiveHeader.kt` | Top app bar with Psilo logo menu |
